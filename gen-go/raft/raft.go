...
type AppendEntriesReq struct {
  ...
  Commands []*Command `thrift:"commands,5" db:"commands" json:"commands"`
...
func (p *AppendEntriesReq) GetCommands() []*Command {
  return p.Commands
}
...
func (p *AppendEntriesReq)  ReadField5(ctx context.Context, iprot thrift.TProtocol) error {
  ...
  for i := 0; i < size; i ++ {
    ...
    p.Commands = append(p.Commands, _elem0)
  }
...
func (p *AppendEntriesReq) writeField5(ctx context.Context, oprot thrift.TProtocol) (err error) {
  if err := oprot.WriteFieldBegin(ctx, "commands", thrift.LIST, 5); err != nil {
    return thrift.PrependError(fmt.Sprintf("%T write field begin error 5:commands: ", p), err) }
  if err := oprot.WriteListBegin(ctx, thrift.STRUCT, len(p.Commands)); err != nil {
  ...
  if err := oprot.WriteFieldEnd(ctx); err != nil {
    return thrift.PrependError(fmt.Sprintf("%T write field end error 5:commands: ", p), err) }
  return err
}
...
func (p *AppendEntriesReq) Equals(other *AppendEntriesReq) bool {
  ...
  if len(p.Commands) != len(other.Commands) { return false }
  for i, _tgt := range p.Commands {
    _src1 := other.Commands[i]
